...
static usize parser_index(const Parser * parser) {
	return nlexer_index(&parser->lexer);
}

static SrcSpan extend_span_to(SrcPos begin, const Parser * parser) {
    return (SrcSpan){
        .pos = begin,
        .len = parser_index(parser) - begin.index,
    };
}
...
static bool parser_eof(Parser * parser) {
	return parser->fst.type == TOKEN_TYPE_EOF;
}

static Token lexer_next_noerr(NLexer * lexer) {
	NLexerTokenResult result;
	while (!(result = nlexer_next(lexer)).ok) {
	}
...
static Token parser_peek2(Parser * parser) {
	return parser->snd;
}

static void parser_advance(Parser * parser) {
	parser->fst = parser->snd;
	parser->snd = lexer_next_noerr(&parser->lexer);
}

static void parser_advance2(Parser * parser) {
	parser_advance(parser);
	parser_advance(parser);	
}

static bool parser_advance_if(Parser * parser, TokenType type, Token * out) {
...
static bool parse_path(Parser * parser, PathBuilder * out) {
    ...
    Token begin; 
	bool global_scope = parser_advance_if(parser, TOKEN_TYPE_SCOPE, &begin);
    if (global_scope) {
        path.is_global = true;
    }
	Token fst_iden;
    if (!parser_advance_if(parser, TOKEN_TYPE_IDENTIFIER, &fst_iden)) {
        return false;
    }
    if (!global_scope) {
        begin = fst_iden;
    ...
    while (parser_advance_if(parser, TOKEN_TYPE_SCOPE, nullptr)) {
        Token iden;
        if (!parser_advance_if(parser, TOKEN_TYPE_IDENTIFIER, &iden)) {
            return false;
        }
...
static bool parse_function_type_parameters(Parser * parser, AstTypeList * out) {
    if (!parser_advance_if(parser, TOKEN_TYPE_LPAREN, nullptr)) {
        return false;
    }
    AstTypeList list = ast_type_list_new(parser->allocator);
    while (!parser_advance_if(parser, TOKEN_TYPE_RPAREN, nullptr)) {
    parse_param:
        ...
        if (!parse_param(parser, &param)) {
            ...
            if (parser_advance_if(parser, TOKEN_TYPE_COMMA, nullptr)) {
                goto parse_param;
            }
            continue;
        }
        ...
        if (parser_advance_if(parser, TOKEN_TYPE_COMMA, nullptr)) {
            goto parse_param;
        } else if (parser_advance_if(parser, TOKEN_TYPE_RPAREN, nullptr)) {
            break;
        } else {
            return false;
        }
...
static AstType parse_function_type(Parser * parser) {
    ...
    Token begin;
    if (!parser_advance_if(parser, TOKEN_TYPE_FN, &begin)) {
        return poisoned;
    }
    AstTypeList list;
    ...
    AstType * return_type;
    if (parser_advance_if(parser, TOKEN_TYPE_COLON, nullptr)) {
...
AstExpr parse_infix_minus(Parser * parser, AstExpr expr) {
    if (!parser_advance_if(parser, TOKEN_TYPE_MINUS, nullptr)) {
        return POISONED_EXPR;
    }
...
AstExpr parse_infix_plus(Parser * parser, AstExpr expr) {
    if (!parser_advance_if(parser, TOKEN_TYPE_PLUS, nullptr)) {
        return POISONED_EXPR;
    }
    AstExpr expr2 = parse_expr_with_precedence(parser, EXPR_PREC_TERM);
    if (!expr_is_ok(&expr2)) {
...
AstExpr parse_infix_dot(Parser * parser, AstExpr expr) {
    if (!parser_advance_if(parser, TOKEN_TYPE_DOT, nullptr)) {
        return POISONED_EXPR;
    }
    PathBuilder path;
    if (!parse_path(parser, &path)) {
...
static AstExpr parse_infix_dotstar(Parser * parser, AstExpr expr) {
    if (!parser_advance_if(parser, TOKEN_TYPE_DOTSTAR, nullptr)) {
        return POISONED_EXPR;
    }
    AstExpr * next = allocator_alloc(parser->allocator, AstExpr);
    if (next == nullptr) {
...
AstExpr parse_infix_as(Parser * parser, AstExpr expr) {
    if (!parser_advance_if(parser, TOKEN_TYPE_AS, nullptr)) {
        return POISONED_EXPR;
    }
...
static bool parse_struct_init(Parser * parser, Path path,
    ...
    Token begin_token;
    if (!parser_advance_if(parser, TOKEN_TYPE_LBRACE, &begin_token)) {
        return false;
    }
    AstExprStructInitParamList list =
        ast_expr_struct_init_param_list_new(parser->allocator);
    forever {
        if (parser_advance_if(parser, TOKEN_TYPE_RBRACE, nullptr)) {
            break;
        }
        if (!parser_advance_if(parser, TOKEN_TYPE_DOT, nullptr)) {
        ...
        Token iden_token;
         if (!parser_advance_if(parser, TOKEN_TYPE_IDENTIFIER, &iden_token)) {
            parse_struct_init_list_recover_invalid_param(parser);
            continue;
        }
       Str iden = token_get_str(iden_token, parser->src);
        if (!parser_advance_if(parser, TOKEN_TYPE_EQ, nullptr)) {
            parse_struct_init_list_recover_invalid_param(parser);
            continue;
        }
        AstExpr expr = parse_expr(parser);
        ...
        if (parser_advance_if(parser, TOKEN_TYPE_COMMA, nullptr)) {
            continue;
        } else if (parser_advance_if(parser, TOKEN_TYPE_RBRACE, nullptr)) {
            break;
        } else {
            return false;
        }
...
static AstExpr parse_prefix_fn(Parser * parser) {
    if (!parser_advance_if(parser, TOKEN_TYPE_FN, nullptr)) {
        return POISONED_EXPR;
    }
    AstParamList params;
...
static AstExpr parse_prefix_lparen(Parser * parser) {
    ...
    if (!parser_advance_if(parser, TOKEN_TYPE_RPAREN, nullptr)) {
        return POISONED_EXPR;
    }
    return expr;
}
...
AstExpr parse_prefix_integer(Parser * parser) {
    Token token;
    if (!parser_advance_if(parser, TOKEN_TYPE_INTEGER, &token)) {
        return POISONED_EXPR;
    }
    usize n = 0;
...
AstExpr parse_prefix_minus(Parser * parser) {
    if (!parser_advance_if(parser, TOKEN_TYPE_AMPERSAND, nullptr)) {
        return POISONED_EXPR;
    }
...
static bool parse_stmt(Parser * parser, AstStmt * out) {
    ...
    case TOKEN_TYPE_RETURN:
        ...
        AstExpr return_expr;
        if (!parser_advance_if(parser, TOKEN_TYPE_SEMICOLON, nullptr)) {
...
static bool parse_param(Parser * parser, AstParam * out) {
    ...
    if (token.type == TOKEN_TYPE_IDENTIFIER &&
        parser_peek2(parser).type == TOKEN_TYPE_COLON) {
        parser_advance2(parser);
...
static bool parse_param_list_after_opening_delimiter(Parser * parser,
    ...
    while (!parser_advance_if(parser, end, nullptr)) {
        if (parser_eof(parser)) {
            return false;
        }
        AstParam param;
        if (!parse_param(parser, &param)) {
            parse_param_list_after_opening_delimiter_recover_invalid_param(
                parser, end);
            parser_advance_if(parser, TOKEN_TYPE_COMMA, nullptr);
            continue;
        }
        ...
        if (parser_advance_if(parser, TOKEN_TYPE_COMMA, nullptr)) {
            continue;
        } else if (parser_advance_if(parser, end, nullptr)) {
            break;
        } else {
            return false;
        }
...
static bool parse_stmt_block(Parser * parser, AstStmtList * out) {
    if (!parser_advance_if(parser, TOKEN_TYPE_LBRACE, nullptr)) {
        return false;
    }
    AstStmtList list = ast_stmt_list_new(parser->allocator);
...
static bool parse_struct(Parser * parser, AstStruct * out) {
    Token begin_token;
    if (!parser_advance_if(parser, TOKEN_TYPE_STRUCT, &begin_token)) {
        return false;
    }
    Token iden_token;
    if (!parser_advance_if(parser, TOKEN_TYPE_IDENTIFIER, &iden_token)) {
...
static bool parse_decl(Parser * parser, AstDecl * out) {
    ...
    bool is_const;
    Token begin;
    if (parser_advance_if(parser, TOKEN_TYPE_LET, &begin)) {
        decl.is_const = false;
    } else if (parser_advance_if(parser, TOKEN_TYPE_CONST, &begin)) {
        decl.is_const = true;
    } else {
        return false;
    }
...
static bool parse_mod(Parser * parser, AstModule * out) {
    Token begin;
    if (!parser_advance_if(parser, TOKEN_TYPE_MOD, &begin)) {
        return false;
    }
    Token iden_token;
    if (!parser_advance_if(parser, TOKEN_TYPE_IDENTIFIER, &iden_token)) {
