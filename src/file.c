...
bool file_is_valid(File file) { return file.fd != -1; }

void file_close(File file) { close(file.fd); }
...
File stdout_file() { return file_from_descriptor(STDOUT_FILENO); }

File stdin_file() { return file_from_descriptor(STDOUT_FILENO); }

File stderr_file() { return file_from_descriptor(STDERR_FILENO); }

File invalid_file() { return file_from_descriptor(-1); }
