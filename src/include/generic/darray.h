...
template(DARRAY_UPPER_NAME) CONCAT(template(DARRAY_LOWER_NAME), _new)(Allocator allocator); \
bool CONCAT(template(DARRAY_LOWER_NAME), _push)(template(DARRAY_UPPER_NAME) array[ref], template(DARRAY_TYPE) value); \
void CONCAT(template(DARRAY_LOWER_NAME), _free)(template(DARRAY_UPPER_NAME) array[ref]);
...
bool CONCAT(template(DARRAY_LOWER_NAME), _push)(template(DARRAY_UPPER_NAME) array[ref], template(DARRAY_TYPE) value) { \
    if (array->size >= array->capacity) { \
        ...
        template(DARRAY_TYPE) * new_alloc = allocator_alloc_n(array->allocator, template(DARRAY_TYPE), new_capacity); \
        if (!new_alloc) { \
            return false; \
        } \
