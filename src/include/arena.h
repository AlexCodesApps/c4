...
static void * arena_alloc_bytes_zeroed(Arena arena[ref], usize size,
                                       usize alignment) {
...
static void * arena_alloc_bytes_n(Arena arena[ref], usize size, usize alignment,
                                  usize n) {
    usize byten;
...
static void * arena_alloc_bytes_zeroed_n(Arena arena[ref], usize size,
                                         usize alignment, usize n) {
    usize byten;
    if (!ckd_mul(size, n, &byten)) {
        return nullptr;
    }
...
#define arena_alloc_uninit_n(arena, type, number)                              \
    ((type *)arena_alloc_bytes_n((arena), sizeof(type), alignof(type),         \
                                 (number)))
...
#define arena_alloc_n(arena, type, number)                                     \
    ((type *)arena_alloc_bytes_zeroed_n((arena), sizeof(type), alignof(type),  \
                                        (number)))
